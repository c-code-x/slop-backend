...
public interface EventCreatorRepository extends JpaRepository<EventCreatorEntity, UUID> {
    @Query("SELECT ec.event FROM EventCreatorEntity ec WHERE ec.creator.owner.id = :userId ORDER BY ec.event.createdAt DESC")
...
//    @Query("SELECT cf.club FROM ClubFollowerEntity cf WHERE cf.follower.user.id = :userId")
}
